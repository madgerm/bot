...
def cmd_hours_list(args: argparse.Namespace) -> int:
    ...
    store = HoursStore(args.root, args.team)
    for record in store.list_diffs(limit=args.limit):
...
def cmd_hours_approve(args: argparse.Namespace) -> int:
    ...
    try:
        service = HoursService.for_team(args.root, args.team)
        record = service.approve(args.diff, args.approved_by)
    except HoursServiceError as exc:
        print(f"Fehler: {exc}", file=sys.stderr)
        return 1
    print(json.dumps(record.to_dict(), indent=2, ensure_ascii=False))
...
def cmd_hours_publish(args: argparse.Namespace) -> int:
    ...
    try:
        service = HoursService.for_team(args.root, args.team)
        record = service.publish(args.diff)
    except HoursServiceError as exc:
        print(f"Fehler: {exc}", file=sys.stderr)
        return 1
    print(json.dumps(record.to_dict(), indent=2, ensure_ascii=False))
...
def cmd_hours_reject(args: argparse.Namespace) -> int:
    ...
    try:
        service = HoursService.for_team(args.root, args.team)
        record = service.reject(args.diff)
    except HoursServiceError as exc:
        print(f"Fehler: {exc}", file=sys.stderr)
        return 1
    print(json.dumps(record.to_dict(), indent=2, ensure_ascii=False))
...
def register_hours_commands(sub, add_root) -> None:
    hours = sub.add_parser("hours", help="Öffnungszeiten (Master, Diff, Publish)")
    hours_sub = hours.add_subparsers(dest="hours_command", required=True)

    show = hours_sub.add_parser("show", help="Master-Öffnungszeiten")
    add_root(show)
    show.add_argument("--team", required=True)
    show.set_defaults(func=cmd_hours_show)

    check = hours_sub.add_parser("check", help="Abgleich Master vs. Website/Google")
    add_root(check)
    ...
    publish = hours_sub.add_parser("publish", help="Nach Freigabe veröffentlichen")
    add_root(publish)
